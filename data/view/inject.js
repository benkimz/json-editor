...
let endpoints = {
  prs: "http://localhost:5123/prs",
  wfs: "http://localhost:5123/wfs"
}, 
endpoint;

let methods = ['GET', 'POST', 'PUT', 'PATCH', "DELETE"];
let requestMethod;
function sendJson(data, request_to_self = false){
  try{
      ...
      if(request_to_self == false){
      xhttp.open(requestMethod, endpoint, true);
      }else{
        xhttp.open(requestMethod, location.href, true);
      }
      ...
      console.error(`"wireframes: << extension >>: ${e}`);
  }
}
...
function buttons() {
  ...
  if (menu) {
    ...
    button.title = 'Persist WireFrame to Database';
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    const p1 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    ...
    button.onclick = e => {
      ...
      try {
        ...
        sendJson(content); // sent json wireframe/ promeroot to api
      }
      catch (e) {}
    };
  }
  ...
  if(menu){
    ...
    for(let key in endpoints){
      ...
      option.value = endpoints[key];
      option.text = `${key} api`;
      select.appendChild(option);
    }
    select.value = endpoints['wfs'];
  ...
  if(menu){
    ...
    select.value = 'PUT';
    select.addEventListener('change', function() {
        requestMethod = this.value;
    });
    menu.insertBefore(select, menu.firstChild);
    requestMethod = select.value;
  }
